...
describe "Variants", type: :feature do
  ...
  context "creating a new variant" do
    it "allows an dash to create a new variant", js: true do
  ...
  context "listing variants" do
    context "there's fewer than 26 variants" do
      it "displays pagination" do
        visit spree.dash_product_variants_path(product)
    ...
    context "there's variants than in preference variants_per_page" do
      ...
      let(:default_per_page) { Spree::Dash::Config[:variants_per_page] }

      it "allows to change number of displayed variants", js: true do
        second_page_link_selector = "a[href='#{spree.dash_product_variants_path(product, page: 2)}']"
        visit spree.dash_product_variants_path(product)
        expect(page).to have_css(second_page_link_selector)
        page.select("50", from: "per_page")
        expect(page).to have_current_path(spree.dash_product_variants_path(product, per_page: 50))
      ...
      it "allows to go to the second page" do
        visit spree.dash_product_variants_path(product)
        page.click_link("2")
        expect(page).to have_current_path(spree.dash_product_variants_path(product, page: 2))
      end
    end
