    ...
    module OrdersHelper
      ...
      def event_links(order, events)
        ...
        events.each do |event|
          ...
          label = Spree.t(event, scope: "dash.order.events", default: Spree.t(event))
          links << link_to_with_icon(label.capitalize, [event.to_sym, :dash, order], icon: event.to_s + ".svg", data: {turbo_method: :put, turbo_confirm: Spree.t(:order_sure_want_to, event: label)}, class: "btn btn-light")
        end
