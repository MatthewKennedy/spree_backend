  ...
  module Dash
    module BaseHelper
      ...
      def flash_alert(flash)
        if flash.present?
          render "spree/dash/shared/toast", message: flash.first[1]
        end
      end
      ...
      def dash_logout_link
        if defined?(dash_logout_path)
          dash_logout_path
      ...
      def product_wysiwyg_editor_enabled?
        Spree::Dash::Config[:product_wysiwyg_editor_enabled]
      end

      def taxon_wysiwyg_editor_enabled?
        Spree::Dash::Config[:taxon_wysiwyg_editor_enabled]
      end
    end
  end
end
