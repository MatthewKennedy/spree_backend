...
function initProductActions () {
  ...
  $('.calculator-fields').each(function () {
    ...
    const $typeSelect = $fieldsContainer.find('.type-select')
    const $settings = $fieldsContainer.find('.settings')
    const $warning = $fieldsContainer.find('.js-warning')
    const originalType = $typeSelect.val()
  ...
  if ($('#promo-rule-option-value-template').length) {
    ...
    const addOptionValue = function (productId, values) {
      ...
      let optionValueId = '#promo-rule-option-value-'
      if (productId) {
