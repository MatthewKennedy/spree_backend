document.addEventListener('spree:load', function () {
  const formFields = $(
    "[data-hook='admin_customer_return_form_fields'], [data-hook='admin_return_authorization_form_fields']"
  )
  ...
  function updateSuggestedAmount () {
    ...
    $.each(checkedItems, function (i, checkbox) {
      ...
      const returnQuantity = parseInt(
        returnItemRow.find('.refund-quantity-input').val(),
        10
      )
      const purchasedQuantity = parseInt(
        returnItemRow.find('.purchased-quantity').text(),
        10
      )
      const amount =
        (returnQuantity / purchasedQuantity) *
        parseFloat(returnItemRow.find('.charged-amount').data('chargedAmount'))
      returnItemRow.find('.refund-amount-input').val(amount.toFixed(2))
      totalPretaxRefund += amount
    })

    const displayTotal = isNaN(totalPretaxRefund)
      ? ''
      : totalPretaxRefund.toFixed(2)
  ...
  if (formFields.length > 0) {
    ...
    formFields.find('input#select-all').on('change', function (ev) {
      const checkBoxes = $(ev.currentTarget)
        .parents('table:first')
        .find('input.add-item')
    ...
    formFields
      .find('.refund-quantity-input')
      .on('keyup mouseup', updateSuggestedAmount)

    formFields
      .find('input, select')
      .not('.add-item')
      .on('change', checkAddItemBox)
  }
})
