    ...
    class TaxonsController < ResourceController
      ...
      def update
        ...
        if successful
          ...
          respond_with(@taxon) do |format|
            format.html { redirect_to spree.edit_dash_taxonomy_taxon_path(@taxonomy.id, @taxon.id) }
          end
        else
          respond_with(@taxon) do |format|
      ...
      def remove_icon
        if @taxon.icon.destroy
          ...
          redirect_to spree.edit_dash_taxonomy_taxon_url(@taxonomy.id, @taxon.id)
        else
      ...
      def successful_reposition_actions
        ...
        respond_to do |format|
          format.turbo_stream { render "spree/dash/taxonomies/reposition" }
        end
      end
      ...
      def location_after_save
        spree.edit_dash_taxonomy_taxon_path(@taxonomy.id, @taxon.id)
      end
