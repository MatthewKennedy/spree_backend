class Spree::Dash::ResourceController < Spree::Dash::BaseController
  include Spree::Dash::Callbacks
  ...
  def destroy
    ...
    if @object.destroy
      ...
      respond_with(@object) do |format|
        format.turbo_stream { render "spree/dash/shared/stream_templates/destroy" }
      end
    else
  ...
  def resource
    ...
    @resource = Spree::Dash::Resource.new controller_path, controller_name, parent_model_name, object_name
  end
  ...
  def new_object_url(options = {})
    if parent_data.present?
      spree.new_polymorphic_url([:dash, parent, model_class], options)
    else
      spree.new_polymorphic_url([:dash, model_class], options)
    end
  end

  def edit_object_url(object, options = {})
    if parent_data.present?
      spree.send "edit_dash_#{resource.model_name}_#{resource.object_name}_url",
        parent, object, options
    else
      spree.send "edit_dash_#{resource.object_name}_url", object, options
    end
  end

  def clone_object_url(object, options = {})
    if parent_data.present?
      spree.send "clone_dash_#{resource.model_name}_#{resource.object_name}_url",
        parent, object, options
    else
      spree.send "clone_dash_#{resource.object_name}_url", object, options
    end
  end
  ...
  def object_url(object = nil, options = {})
    ...
    if parent_data.present?
      spree.send "dash_#{resource.model_name}_#{resource.object_name}_url", parent, target, options
    else
      spree.send "dash_#{resource.object_name}_url", target, options
    end
  end

  def collection_url(options = {})
    if parent_data.present?
      spree.polymorphic_url([:dash, parent, model_class], options)
    else
      spree.polymorphic_url([:dash, model_class], options)
    end
  end
