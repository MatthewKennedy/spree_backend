module Spree
  module Dash
    class OrdersController < Spree::Dash::BaseController
      ...
      def index
        params[:q] ||= {}
        params[:q][:completed_at_not_null] ||= "1" if Spree::Dash::Config[:show_only_complete_orders_by_default]
        ...
        @orders = @search.result(distinct: true)
          .page(params[:page])
          .per(params[:per_page] || Spree::Dash::Config[:dash_orders_per_page])
      ...
      def update
        if @order.update(permitted_resource_params)
          ...
          redirect_back fallback_location: spree.edit_dash_order_url(@order)
      ...
      def open_adjustments
        ...
        redirect_back fallback_location: spree.dash_order_adjustments_url(@order)
      end
